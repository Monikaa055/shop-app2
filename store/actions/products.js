...
export const fetchProducts = () => {
    return async dispatch => {
        ...
        try {
            const response = await fetch('https://rn-complete-guide-41fbb.firebaseio.com/products.json');
            ...
            //send to custom anayltics server
            throw err;
        }
...
export const updateProduct = (id, title, description, imageUrl) => {
    return async dispatch => {
        const response = await fetch(
            `https://rn-complete-guide-41fbb.firebaseio.com/products/${id}.json`,
            {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json'
